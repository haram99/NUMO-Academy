...
# ---------------------------------------------------------
# تنسيقات CSS: دعم الشاشات الصغيرة (الجوال) ومنع اقتصاص المحتوى
# ---------------------------------------------------------
st.markdown(
    f"""
    <style>
    ...
    /* تعديل الهوامش لتناسب الهواتف والشاشات الصغيرة */
    .block-container {{
...
if page == "🏠 الرئيسية":
    ...
    user_query = st.text_input("📝 أدخل استفسارك التقني أو الكلمة المفتاحية:", placeholder="مثال: الأمن السيبراني، سحابة")
    if user_query:
    ...
    with st.form("competition_form"):
        ...
        q1 = st.radio("1. أي مما يلي يُعتبر المكون الأساسي الذي يحاكي الشبكات العصبية للبشر؟", ["الخوارزميات التقليدية", "الشبكات العصبية العميقة (Deep Neural Networks)", "جداول البيانات الثابتة"], index=None)
        q2 = st.radio("2. ما هي الفائدة الرئيسية للحوسبة السحابية للشركات؟", ["الاعتماد كليًا على الخوادم المحلية", "مرونة الوصول للموارد وخفض التكاليف التشغيلية", "إبطاء معالجة البيانات"], index=None)
        q3 = st.radio("3. أي من الآتي يمثل ممارسة صحيحة في الأمن السيبراني الشخصي؟", ["استخدام كلمة مرور موحدة وسهلة", "تفعيل المصادقة الثنائية (2FA) وتحديث البرمجيات دورياً", "النقر على الروابط غير المعروفة"], index=None)
...
# ---------------------------------------------------------
# تنسيقات CSS: تصميم متناسق وتجسيم الشعار
# ---------------------------------------------------------
st.markdown(
    f"""
    <style>
    ...
    with st.form("competition_form"):
        ...
        if submitted:
            if not participant_name.strip() or not participant_phone.strip():
                ...
                # إرسال البيانات مباشرة إلى جدول Google Sheets السحابي
                try:
                    payload = {
                        "name": participant_name,
                        "phone": participant_phone,
