...
# ---------------------------------------------------------
# تنسيقات CSS: دعم الجوال وتنسيق الواجهة
# ---------------------------------------------------------
st.markdown(
    f"""
    <style>
...
# ---------------------------------------------------------
# الشريط الجانبي (مع إضافة key فريد لمنع تكرار المعرّف)
# ---------------------------------------------------------
with st.sidebar:
    if logo_base64:
        st.markdown(
    ...
    page = st.radio(
        ...
        key="main_navigation"  # <-- مفتاح فريد لمنع تكرار العناصر
    )
    ...
    if logo_base64:
        st.markdown(
            ...
            <div class="sphere-logo-container" style="margin: 0;">
                <img src="data:image/jpeg;base64,{logo_base64}" class="sphere-logo" style="width: 70px; height: 70px;">
            </div>
            """,
            unsafe_allow_html=True,
        )
...
with col2:
    ...
    st.markdown("<p style='text-align: center; color: #94a3b8; font-size: 13px;'>برنامج إثراء مهارات المستقبل للمرحلة الثانوية — أكاديمية نمو العالي</p>", unsafe_allow_html=True)
...
if page == "🏠 الرئيسية":
    ...
    user_query = st.text_input("📝 أدخل استفسارك التقني أو الكلمة المفتاحية:", placeholder="مثال: الأمن السيبراني، سحابة", key="lab_query_input")
    if user_query:
    ...
    with st.form("competition_form"):
        ...
        q1 = st.radio("1. أي مما يلي يُعتبر المكون الأساسي الذي يحاكي الشبكات العصبية للبشر؟", ["الخوارزميات التقليدية", "الشبكات العصبية العميقة (Deep Neural Networks)", "جداول البيانات الثابتة"], index=None, key="q1_radio")
        q2 = st.radio("2. ما هي الفائدة الرئيسية للحوسبة السحابية للشركات؟", ["الاعتماد كليًا على الخوادم المحلية", "مرونة الوصول للموارد وخفض التكاليف التشغيلية", "إبطاء معالجة البيانات"], index=None, key="q2_radio")
        q3 = st.radio("3. أي من الآتي يمثل ممارسة صحيحة في الأمن السيبراني الشخصي؟", ["استخدام كلمة مرور موحدة وسهلة", "تفعيل المصادقة الثنائية (2FA) وتحديث البرمجيات دورياً", "النقر على الروابط غير المعروفة"], index=None, key="q3_radio")
